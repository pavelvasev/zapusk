...
Предназначение: позволяет создать общий разделяемый ресурс, который 
* не будет подвергнут destroy пока не исчезнут все его запуск-пользователи (т.е. какие-либо развернутые компоненты запуск-программ).
* при разворачивании ресурса устраиваются всеобщие выборы. победивший перехватывает управление всех остальных.
таким образом достигается консистентность параметров - будут действовать параметры победителя.

Параметры:
* **dir** - каталог хранения файлов флагов, для отслеживания состояния. По умолчанию `/zapusk/_guards`.
* **key** - имя флага. Обязательный.
* **priority** - приоритет на выборах, чем меньше тем главнее. Необязательный, по умолчанию 20.
